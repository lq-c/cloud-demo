  ...
  <div id="vol-container" :class="['vol-theme-' + theme]">
    <div class="vol-aside" :style="{ width: menuWidth + 'px' }">
      <div class="header" :style="{ width: menuWidth - 1 + 'px' }">
        <img style="width: 160px;height: 33px; margin-top: 13px;" v-show="!isCollapse" v-bind:src="logo" />
        <i @click="toggleLeft" class="el-icon-s-fold collapse-menu" />
      </div>
      <div class="vol-menu">
        <el-scrollbar style="height: 100%">
          <VolMenu @openPage="openPage" :isCollapse="isCollapse"></VolMenu>
        </el-scrollbar>
      </div>
    </div>
    <div class="vol-container" :style="{ left: menuWidth - 1 + 'px' }">
      <div class="vol-header">
        <div class="project-name">
          ...
          <el-dropdown trigger="click">
            <span class="el-dropdown-link">
              <!-- <img :src="projectStore.currentProject?.icon || '@/assets/imgs/project/default-project-avatar.png'" -->
              <img v-if="projectStore.currentProject && projectStore.currentProject?.icon"
                :src="projectStore.currentProject.icon"
                style="width: 32px;height: 32px;margin-right: 8px;border-radius: 4px" alt="">
              <img v-else src="@/assets/imgs/project/default-project-avatar.png"
                style="width: 32px;height: 32px;margin-right: 8px;border-radius: 4px" alt="">
              {{ projectStore.currentProject?.name || '请选择项目' }}<el-icon class="el-icon--right"><arrow-down /></el-icon>
            </span>
            <template #dropdown>
              <el-dropdown-menu style="padding-bottom: 44px;">
                <!-- <template v-if="projectStore?.list?.length != 0"> -->
                <el-dropdown-item v-for="(item, index) in projectStore.list"
                  @click="projectStore.setCurrentProject(item)">
                  <img :src="item.icon" style="width: 24px;height: 24px;margin-right: 6px;border-radius: 4px;" alt="">
                  {{ item.name }}
                </el-dropdown-item>
                <el-dropdown-item style="height: 43px;"></el-dropdown-item>
                <el-dropdown-item class="toproject" @click="ToProject()">
                  <img src="@/assets/imgs/project/default-project-avatar.png"
                    style="width: 24px;height: 24px;margin-right: 6px;" alt="">
                  项目库
                </el-dropdown-item>
              </el-dropdown-menu>
            </template>
        ...
        <div class="header-info">
          ...
          <div class="user">
            <span style="margin-bottom: 4px;">{{ userStore.userInfo.principal_name }}</span>
      ...
      <div class="vol-path">
        <el-tabs @tab-click="selectNav" @tab-remove="removeNav" @contextmenu.prevent="bindRightClickMenu(false)"
          type="card" class="header-navigation" v-model="selectId" :strtch="false">
      ...
      <div class="vol-main" id="vol-main">
        ...
        <!-- <el-scrollbar style="height: 100%"> -->
        <loading v-show="commonSrore.isLoading"></loading>
...
// var imgUrl = require("@/assets/imgs/cloud-logo.svg");
import imgUrl from '@/assets/imgs/cloud-logo.svg'
import error_img from '@/assets/imgs/error-img.png'
var $this;
var $interval;
var $indexDate;
import {
...
export default defineComponent({
  ...
  setup (props, context) {
    ...
    const commonSrore = useCommonStore();
    // const {  } = storeToRefs(userStore)
    // 获取全局属性和方法
    const { proxy } = getCurrentInstance();

    // 菜单导航默认宽度
    const menuWidth = ref(244);
    const contextMenuVisible = ref(false);
    const isCollapse = ref(false);
    const drawer_model = ref(false);
    const messageModel = ref(false);
    const theme_color = ref([
      { name: "blue", color: "#0F2D7D", leftColor: "#071C58" },
      { name: "dark", color: "#272929", leftColor: '#1F1F1F' }
    ]);
    const links = ref([
      // {
      //   text: "框架文档",
    ...
    const toggleLeft = () => {
      ...
      menuWidth.value = isCollapse.value ? 63 : 244;
    };
    ...
    watch(
      ...
      (newVal, oldVal) => {
        ...
        if (newVal) {
          ...
          document.bodyNaNpxoveEventListener("click", closeTabsMenu);
        }
      }
    );
    ...
    const created = () => {
      ...
      let _userInfo = userStore.userInfo;
      if (_userInfo) {
      ...
      //   console.log('menu tree', data, router);
      //   menuOptions.value = data;
      //   // menuOptions.value = router.options.routes;
