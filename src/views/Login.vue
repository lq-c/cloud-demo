<template>
  <div class="login-container">
    <div class="project-name">泓科晟睿</div>
...
export default defineComponent({
  setup (props, context) {
    ...
    const localAuthenticationProviderId = ref('')


    const getVierificationCode = async () => {
      let result = (await http.get('/api/user-center/authentication')).data;
      localAuthenticationProviderId.value = result.filter(x => !x.disabled && x.type == 'Local')[0]
    };
    ...
    const login = async () => {
      ...
      try {
        loading.value = true;
        let result = await http.post(`/api/user-center/localauth/${localAuthenticationProviderId.value.id}`, userInfo, '正在登录....');
